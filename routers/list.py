...
async def delete_list(db:db_dependency , user:user_dependency , list_name:str):
    ...
    if not list:
        raise HTTPException(status_code=404 , detail=f'there\' no list called {list_name}')
...
async def update_list(db:db_dependency, user: user_dependency , list_name:str ,updated_list:list_request):
    ...
    exsiting_shared_with = list.shared_with or [] # exsisitng shared list or empty list if none 
...
async def get_shared_with_me(db: db_dependency, user: user_dependency):
    shared_lists = db.query(Lists).filter(text(":username = ANY(shared_with)").params(username=user.get('username'))).all()
